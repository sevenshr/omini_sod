...
class OminiModel(L.LightningModule):
    def __init__(
        ...
        for dataset_name in val_datasets_name:
            self.val_metrics[dataset_name] = {
                "m1": utils.DDPAverager(),
                "m2": utils.DDPAverager(),
                "m3": utils.DDPAverager(),
                "m4": utils.DDPAverager(),
                "m5": utils.DDPAverager(),
            }
    ...
    def test_step(self, batch, batch_idx, dataloader_idx):
        ...
        for i in range(1000):
            ...
            position_scales.append(batch.get(f"position_scale_{i}", [1.0])[0])
            latent_masks.append(batch.get(f"condition_latent_mask_{i}", None))
            conditions_use.append(Condition(conditions[-1], adapter, position_deltas[-1][0], position_scales[-1]))
        ...
        metrics["m1"].update(torch.tensor(result1.item()), bs)
        metrics["m2"].update(torch.tensor(result2.item()), bs)
        metrics["m3"].update(torch.tensor(result3.item()), bs)
        metrics["m4"].update(torch.tensor(result4.item()), bs)
        metrics["m5"].update(torch.tensor(result5.item()), bs)
...
class TrainingCallback(L.Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        for dataset_name, metrics in pl_module.val_metrics.items():
            m1 = metrics["m1"].compute().cpu().numpy()
            m2 = metrics["m2"].compute().cpu().numpy()
            m3 = metrics["m3"].compute().cpu().numpy()
            m4 = metrics["m4"].compute().cpu().numpy()
            m5 = metrics["m5"].compute().cpu().numpy()
